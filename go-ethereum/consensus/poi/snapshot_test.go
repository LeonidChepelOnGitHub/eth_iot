...
// checkpoint creates a Poi checkpoint signer section from the provided list
...
// sign calculates a Poi digital signature for the given block and embeds it
...
// the account may or may not have cast a Poi vote.
...
// Tests that Poi signer voting is evaluated correctly for various simple and
...
func (tt *poiTest) run(t *testing.T) {
	...
	config.Poi = &params.PoiConfig{
		Period: 1,
		Epoch:  tt.epoch,
	}
	genesis.Config = &config

	engine := New(config.Poi, rawdb.NewMemoryDatabase())
	...
	// Pass all the headers through poi and ensure tallying succeeds
